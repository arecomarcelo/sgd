...
st.set_page_config(
    page_title="Gerenciar Dashboards",
    page_icon="⚙️",
    layout="wide"
)
    ...
    st.dataframe(
        ...
        column_config={
            "Ordem": st.column_config.NumberColumn(
                "Ordem",
                help="Ordem de exibição no slideshow",
                width="small"
            ),
            ...
            "Duração": st.column_config.TextColumn(
                "Duração",
                help="Tempo de exibição em segundos",
                width="small"
            )
        }
    )
...
def ajustar_ordens(dashboard_id, nova_ordem, ordem_antiga):
    ...
    """
    configs = Dashboard_Config.objects.exclude(Dashboard__id=dashboard_id).order_by('Ordem')
        ...
        with st.expander(f"{'✅' if dash.Ativo else '❌'} {dash.Nome}", expanded=False):
            ...
            try:
                ...
                with col1:
                    ...
                    st.write(f"**Status:** {'Ativo ✅' if dash.Ativo else 'Inativo ❌'}")
                ...
                with col3:
                    ...
                    if st.button(
                        ...
                        dash.save()
                        st.success(f"Dashboard {'ativado' if dash.Ativo else 'desativado'} com sucesso!")
                        st.rerun()
                    ...
                    if st.button("💾 Salvar", key=f"save_{dash.id}"):
                        ...
                        st.success(f"✅ Dashboard '{dash.Nome}' atualizado com sucesso!")
                        st.rerun()

            except Dashboard_Config.DoesNotExist:
                st.warning("⚠️ Sem configuração de exibição")
                st.info("💡 Este dashboard precisa de uma configuração para ser exibido no slideshow")
else:
    st.info("📭 Nenhum dashboard cadastrado ainda.")
    st.warning("⚠️ A funcionalidade de cadastro foi desabilitada. Entre em contato com o administrador.")

st.markdown("---")
st.caption("💡 **Dica**: Ajuste a ordem e duração dos dashboards conforme necessário. Clique em 'Salvar' para aplicar as alterações.")
